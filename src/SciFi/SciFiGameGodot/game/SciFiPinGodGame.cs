...
public class SciFiPinGodGame : PinGodGame
{
    ...
    {
        var pl = GetSciFiPlayer();

        //double playfield spawn turned on
        if (pl.SpawnEnabled > GameOption.Off)
            points *= 2;
    ...
    internal Resource GetResource(string resName)
    {
		return GetResources()?.GetResource(resName) ?? null;
    }
