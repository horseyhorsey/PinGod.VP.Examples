...
'Const GameDirectory = ".\PinGod.SpinningDiscs.exe" 'exported game

'Debug
Const IsDebug = True
Const GameDirectory = "..\SpinningDiscsGodot" ' Loads the godot pingod game project
Const UseSolenoids = 1 ' Check for solenoid states?
Const UsePdbLeds = 1  ' use led (color)
Const UseLamps = 0  ' Check for lamp states?
Dim bsTrough, bsSaucer, plungerIM, spinningDisc, swSaucer : swSaucer = 27
...
Sub InitGame
  ...
  ' Auto Plunger
	...
	' SPINNING DISC - cvpmTurntable
	Set spinningDisc = New cvpmTurnTable
		spinningDisc.InitTurnTable SpinningDiscTrigger, 20
		spinningDisc.SpinCW = True
		spinningDisc.maxSpeed = 140
		spinningDisc.spinUp = 20
		spinningDisc.spinDown = 5
		spinningDisc.CreateEvents "spinningDisc"
		spinningDisc.MotorOn = False
...
Sub SolDiscMotor2(Enabled)	
	SolDiscMotor(Enabled)
End Sub

Sub SolDiscMotor(Enabled)

	' Debug.Print "motor running " & Enabled	
	spinningDisc.MotorOn = Enabled	

	If Enabled Then
		stepAngle			= 20.0
		discsAreRunning		= True
		stopDiscs			= False
		SpinningDiscTimer.Interval = 10
		SpinningDiscTimer.Enabled = True
	Else
		stopDiscs			= True
		discsAreRunning		= True
	End If
...
Sub SpinningDiscTimer_Timer()
	...
	End If
	' rotate discs
	SpinningDiscPrimitive.RotAndTra2 = 360 - discAngle
	' Debug.Print "rotated disc"
	If stopDiscs Then
		stepAngle = stepAngle - 0.1
		If stepAngle <= 0 Then
			SpinningDiscTimer.Enabled 	= False
		End If
	End If
End Sub
...
SolCallback(5) = "SolDiscMotor" ' Spinning disc coil
